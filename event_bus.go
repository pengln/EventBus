...
func (bus *EventBus) doPublish(handler *eventHandler, topic string, args ...interface{}) {
	// fix: Handler can't be released automatically when remote client disconnects, resulting
	// in null pointer when sending message, causing server-side panic. If an exception occurs
	// when `Call` is called, the handler will be removed from the subscription queue, preventing
	// null pointer panic from being triggered.
	defer func() {
		if r := recover(); r != nil {
			i := bus.findHandlerIdx(topic, handler.callBack)
			bus.removeHandler(topic, i)
		}
	}()
